...
// Online tables auto-close after 72h of inactivity (overridable for tests).
const EXPIRE_MS = () => (typeof window.__expireMs === 'number' ? window.__expireMs : 72*60*60*1000);
...
function openMap(){
  if (!S.uid || !S.profile || S.profile.provider === 'guest'){
   // openOnline preloads the auth SDK, so the Google button can open its
   // popup within the tap gesture (required on iOS Safari).
   openOnline();
    return;
  }
  show('map');
...
function ensureAuthWatcher(){
  ...
  const code=(e&&e.code)||'';
  const msgs={
    'auth/popup-blocked':'The browser blocked the sign-in window. Tap Continue with Google again, or allow pop-ups for this site in the browser settings.',
    'auth/popup-closed-by-user':'The sign-in window closed before finishing. Please try again.',
    'auth/cancelled-popup-request':'Another sign-in attempt was already open. Please try again.',
    'auth/unauthorized-domain':'This web address is not on the Firebase authorized-domains list, so Google sign-in is refused here.',
    'auth/network-request-failed':'Network error during sign-in. Check your connection and try again.'
  };
  return msgs[code]||(e&&e.message)||'Sign-in failed. Please try again.';
}
async function signInGoogle(){
  const err=$('auth-err'); if(err) err.classList.add('hidden');
  try{
    // On iPhone/iPad Safari the popup is only allowed while the tap gesture is
    // still live, so these loads must already be warm (openOnline preloads
    // them the moment the sign-in panel appears).
    await loadFirebase();
    await loadFirebaseAuth(); // the auth SDK is only needed for Google
    ensureAuthWatcher();
    await firebase.auth().signInWithPopup(new firebase.auth.GoogleAuthProvider());
  }catch(e){ authError(friendlyAuthMessage(e)); }
}
...
function recordMatchEnd(room){
  ...
  if(room.result==='closed'){ // neutral close: no winner, loser, or tie recorded
...
function renderPublicRooms(){
  ...
  const now=Date.now();
  const expire=EXPIRE_MS();
  const entries=Object.entries(S.publicRooms||{})
    .filter(([code,v])=>{
      if(!v) return false;
      const idle=now-(v.lastActive||v.createdAt||0);
      if(idle>=expire){ if(fdb) fdb.ref('publicRooms/'+code).remove().catch(()=>{}); return false; }
      return true;
    })
    .sort((a,b)=>(b[1].lastActive||b[1].createdAt||0)-(a[1].lastActive||a[1].createdAt||0));
...
async function createTable({size, seatCfg, visibility}){
  ...
  await db.ref('rooms/'+code).set({
    createdAt:Date.now(), lastActive:Date.now(), size, players,
    rounds:{}, currentRound:0, turn:0, totals,
    status:'waiting', visibility
  });
  if (visibility==='public') await db.ref('publicRooms/'+code).set({host:S.myName,createdAt:Date.now(),lastActive:Date.now(),size});
  return code;
}
...
/* Decide the result when a table closes without anyone reaching 29.
   Unique leader -> that seat wins. Otherwise (all tied, including 0/0) ->
   neutral close: 'closed' records no winner, loser, or tie. */
function resolveByLead(room,size){
  let best=-Infinity, bestSeat=0, bestCount=0;
  for(let k=0;k<size;k++){
    const t=(room.totals&&room.totals['t'+k])||0;
    if(t>best){ best=t; bestSeat=k; bestCount=1; } else if(t===best) bestCount++;
  }
  return bestCount===1 ? 'p'+bestSeat : 'closed';
}
/* A seated player forfeits: the game ends and the win is awarded to the
   leading opponent (forfeiting player is the loser). */
function forfeitGame(){
  const room=S.room;
  if(!room || room.status!=='playing' || S.seat<0) return;
  const size=room.size||2;
  let best=-Infinity, winner=-1;
  for(let k=0;k<size;k++){
    if(k===S.seat) continue;
    const t=(room.totals&&room.totals['t'+k])||0;
    if(t>best){ best=t; winner=k; }
  }
  if(winner<0) winner=(S.seat+1)%size;
  fdb.ref('rooms/'+S.code).update({status:'done', result:'p'+winner, forfeitedBy:S.seat, lastActive:Date.now()}).catch(()=>{});
  fdb.ref('publicRooms/'+S.code).remove().catch(()=>{});
}
...
function onRoomUpdate(){
  ...
  // Auto-close: a table idle for 72h resolves and shuts down.
  if(room.status!=='done'){
    const idle=Date.now()-(room.lastActive||room.createdAt||0);
    if(idle>=EXPIRE_MS()){
      fdb.ref('rooms/'+S.code).update({status:'done', result:resolveByLead(room,size), closedIdle:true, lastActive:Date.now()}).catch(()=>{});
      fdb.ref('publicRooms/'+S.code).remove().catch(()=>{});
      return;
    }
  }
  ...
  if(S.seat===0 && room.status==='waiting'){
    ...
    if(full){
      fdb.ref('rooms/'+S.code).update({status:'playing', lastActive:Date.now()});
      if(room.visibility==='public') fdb.ref('publicRooms/'+S.code).update({status:'playing', lastActive:Date.now()}).catch(()=>{});
      return;
    }
  }
  ...
  if(S.seat===0 && room.status==='playing'){
    ...
    if(all){
      ...
      const updates={ totals, lastActive:Date.now() };
      if(anyHome){
...
function renderOnline(){
  ...
  const fbtn=$('o-forfeit');
  if(fbtn) fbtn.classList.toggle('hidden', !(room.status==='playing' && S.seat>=0));

  if(room.status==='done'){
    recordMatchEnd(room);
    if(room.result==='closed'){
      $('do-title').textContent='Table closed';
      $('do-sub').textContent='No result \u2014 the table sat idle too long.';
    } else {
      const iWon=S.seat>=0 && room.result==='p'+S.seat;
      const note = room.forfeitedBy!==undefined
        ? (room.forfeitedBy===S.seat ? ' \u00b7 you forfeited' : ` \u00b7 ${seatName(room,room.forfeitedBy)} forfeited`)
        : '';
      $('do-title').textContent= iWon?'You win the table':`${seatName(room,+room.result.slice(1))} takes it`;
      $('do-sub').textContent=lanes.map(l=>`${l.name.replace(' (you)','')} ${l.total}`).join(' · ')+note;
    }
...
{ const fb=$('o-forfeit'); if(fb) fb.addEventListener('click',()=>{
    if(confirm('Forfeit this game? Your opponent will be awarded the win.')) forfeitGame();
  }); }
